//prod-GOOGLE_SHEET_GET_ALL_PLAYER_GAMES = "https://script.google.com/macros/s/AKfycbyGFvXOCaImTwo3EkTQ4bLkQFZp3ocabGB0R5lMLI0mtBKHuRqwVriT4rkRv-WpNDA4Cg/exec";
GOOGLE_SHEET_GET_ALL_PLAYER_GAMES = "https://script.google.com/macros/s/AKfycbyvK8814kEShrRyiyLMbWvfHPMOjVct10v6lW4ktKEu9IPGv8Kzq2VnzNJe45ZKNAhcEA/exec";
...
function findCoalitionImageUrl(score, game){
  let src = 'img/generic-meeple.png';
  let x = score.toLowerCase();
  game.players.forEach(player=>{
    if(x.includes(player.faction.toLowerCase())){
      src ="img/"+ factionImages[player.faction];
    }
  })
  return src;
}
function findDomImageUrl(score){
  let x = score.toLowerCase();
  let v;
  if(x.includes('bun') || x.includes('rab')){
    v ="img/"+ domImages['Bunny'];
  }else if(x.includes('bird')){
    v = "img/"+domImages['Bird'];
  }else if(x.includes('fox')){
    v ="img/"+ domImages['Fox'];
  }else if(x.includes('mouse')){
    v ="img/"+ domImages['Mouse'];
  }else{
    v = 'img/generic-meeple.png';
...
function loadPlayerGames(){
  ...
  allGames.forEach(game=>{
    if(gameMatchesFilter(game, filter)) {
      ...
      tr.innerHTML = rowTemplate(transformGame(game, idx++));
      tbody.append(tr);
    }
  });
...
function transformGame(game, idx){
  game.count = idx;
  game.srcUrl = 'https://docs.google.com/spreadsheets/d/1xqb3vdvE5tbbVfa1eUFvx5uYIO52pePNLfOA3NYR2eE/edit#gid=1595954131&range='+game.id+":"+game.id;
  game.recordedDate = new Date(game.timeStamp).toLocaleDateString();
  game.mapImgSrc = 'img/'+ mapImages[game['map']];
  game.deckImgSrc = 'img/'+ deckImages[game.deck];
  game.players = [game.player1, game.player2, game.player3, game.player4];
  game.players.forEach(player=>{
    transformPlayer(player, game);
  });
  return game;
}
function transformPlayer(player, game){
  if(isNaN(player.score)){
    if(player.score.toLowerCase().includes('dom')){
      player.isDom = true;
      player.domImageUrl = findDomImageUrl(player.score);
    }else if (player.score.toLowerCase().includes('coal')){
      player.isCoalition = true;
    player.coalitionImageUrl = findCoalitionImageUrl(player.score, game);
    }
  }else{
    player.isPoint = true;
  }
  player.isWinner = player.leagueScore > 0;
  player.factionImageUrl = 'img/'+ factionImages[player.faction];
  return player;
}
...
window.addEventListener("load", () => {
  let source = document.getElementById('gameRowTemplate');
  rowTemplate = Handlebars.compile(source.innerHTML);
